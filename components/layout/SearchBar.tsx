        ...
        <div className="flex-1 max-w-md relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute top-full mt-3 w-full bg-[#161618] border border-white/10 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden z-[100] animate-in fade-in slide-in-from-top-4 duration-200">
                    ...
                    <div id="search-results-list" role="listbox" className="max-h-[400px] overflow-y-auto custom-scrollbar">
                        {results.length > 0 ? (
                            results.map((match, index) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 p-4 transition-colors text-left group/item ${activeIndex === index ? 'bg-white/10 border-l-2 border-primary' : 'hover:bg-white/5'
                                        }`}
                                >
