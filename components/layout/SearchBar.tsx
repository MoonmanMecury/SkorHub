...
export function SearchBar() {
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (!isOpen || results.length === 0) {
            if (e.key === 'ArrowDown' && query.length >= 2) setIsOpen(true);
            return;
        }
        if (e.key === 'ArrowDown') {
            e.preventDefault();
                        ...
                        {results.length > 0 ? (
                            results.map((match, index) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 p-4 transition-colors text-left group/item ${index === activeIndex ? 'bg-primary/20 border-l-2 border-primary' : 'hover:bg-white/5 border-l-2 border-transparent'}`}
                                >
