    ...
    return (
        <div className="block group relative">
            ...
            <button
                ...
                className={`absolute top-2.5 right-2.5 w-7 h-7 rounded-xl border flex items-center justify-center transition-all z-20 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none ${isFav
                    ? 'bg-primary text-white border-primary/50'
                    : 'bg-black/40 text-white/50 border-white/10 hover:text-yellow-400 backdrop-blur-md opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100'
                    }`}
            >
                <span className={`material-symbols-outlined text-[16px] ${isFav ? 'fill-current' : ''}`} aria-hidden="true">
                    star
                </span>
            </button>
