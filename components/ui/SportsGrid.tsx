...
export function SportsGrid({ sports }: SportsGridProps) {
    ...
    return (
        <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-6">
            {list.map((cat, i) => (
                <Link key={i} href={`/category/${cat.id}`} className="flex-none group cursor-pointer block rounded-[2rem] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-4 focus-visible:ring-offset-background-dark">
