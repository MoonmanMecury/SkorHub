    ...
    return (
        <div className="group relative bg-[#161618]/40 hover:bg-[#161618] border border-white/5 hover:border-primary/20 rounded-2xl p-4 transition-all duration-300">
            ...
            <button
                ...
                className={`absolute -right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg border flex items-center justify-center transition-all outline-none focus-visible:opacity-100 focus-visible:translate-x-2 focus-visible:ring-2 focus-visible:ring-primary ${isFav ? 'bg-primary text-white border-primary/50 opacity-100 translate-x-2' : 'bg-[#0A0A0B] text-slate-500 border-white/10 opacity-0 group-hover:opacity-100 group-hover:translate-x-2'
                    }`}
            >
                <span className={`material-symbols-outlined text-sm ${isFav ? 'filled' : ''}`}>star</span>
            </button>
        </div>
    );
}
