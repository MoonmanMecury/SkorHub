...
export async function POST(request: NextRequest) {
    try {
        ...
        // Fail-secure: Reject if hash key is not configured
        if (!hashKey) {
            console.error('[Webhook] Missing LENCO_WEBHOOK_HASH_KEY');
            return NextResponse.json({ error: 'Configuration error' }, { status: 500 });
        }

        // Fail-secure: Reject if signature header is missing
        if (!signature) {
        ...
        // Parse JSON ONLY after successful signature verification to prevent DoS via large/malicious payloads
