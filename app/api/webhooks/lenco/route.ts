...
export async function POST(request: NextRequest) {
    try {
        ...
        // 1. Signature Verification - Mandate secret and signature header
        const hashKey = process.env.LENCO_WEBHOOK_HASH_KEY;
        const signatureHeader = request.headers.get('x-lenco-signature');

        if (!hashKey || !signatureHeader) {
            console.error('[Webhook] Missing verification credentials');
        ...
        if (tier === 'supporter' || tier === 'vip') {
            // Use parameterized CASE to avoid SQL string interpolation
            await db.query(`
                UPDATE users
