...
export async function POST(request: NextRequest) {
    try {
        ...
        if (tier === 'supporter' || tier === 'vip') {
            // Security: Use parameterized query for expiresAt to prevent SQL injection
            // Using a raw SQL fragment like "NOW() + INTERVAL '30 days'" in string interpolation is dangerous
            // if any part of it was user-controlled.
            if (payment.is_recurring) {
                await db.query(`
                    ...
                    SET supporter_tier = $1,
                        ...
                        supporter_expires_at = NOW() + INTERVAL '30 days'
                    WHERE id = $3
