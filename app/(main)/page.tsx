...
export default async function Home() {
    // Parallelize data fetching to minimize latency
    ...
    // BOLT OPTIMIZATION:
    // Use a Set for O(1) lookup to improve filtering performance from O(N*M) to O(N)
    const liveMatchIds = new Set(liveMatches.map(l => l.id));
    const upcomingAndRecent = allMatches
        .filter(m => !liveMatchIds.has(m.id))
        .slice(0, 18);

    // Group matches by category
    const matchesByCategory = allMatches.reduce((acc: { [key: string]: typeof allMatches }, match) => {
