...
export default async function Home() {
    // Parallelize data fetching to avoid waterfall and improve TTFB
    ...
    // Group matches by category using a single pass for efficiency
    ...
    // Using a Set for O(1) lookup to improve filtering performance from O(N*M) to O(N+M)
    const liveMatchIds = new Set(liveMatches.map(m => m.id));
    const upcomingMatches = allMatches.filter(m => !liveMatchIds.has(m.id)).slice(0, 18);
