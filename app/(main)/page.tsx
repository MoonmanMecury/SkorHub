...
export default async function Home() {
    // Parallelize data fetching to resolve waterfalls and improve page load time
    ...
    // Use a Set for O(1) lookup to optimize filtering of live matches from the all matches list
    const liveIds = new Set(liveMatches.map(l => l.id));
    const upcomingMatches = allMatches.filter(m => !liveIds.has(m.id));
