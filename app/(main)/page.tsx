...
export default async function Home() {
    // Parallelize data fetching to eliminate network waterfall and reduce TTFB
    ...
    // Optimize lookups and filtering: use a Set for O(1) live match check
    // This improves performance from O(N*M) to O(N+M) when filtering upcoming matches
    const liveMatchIds = new Set(liveMatches.map(m => m.id));
    const upcomingMatches = allMatches.filter(m => !liveMatchIds.has(m.id));

    // Group matches by category for efficient rendering of sport sections
