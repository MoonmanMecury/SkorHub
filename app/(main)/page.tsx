...
export default async function Home() {
    // Parallelize data fetching to reduce total load time (Bolt optimization)
    ...
    // O(1) lookup for live match IDs to optimize filtering in "Upcoming & Recent" section
    const liveMatchIds = new Set(liveMatches.map(m => m.id));

    // Group matches by category for efficient display
    const matchesByCategory = allMatches.reduce<Record<string, typeof allMatches>>((acc, match) => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 pt-2 pb-10 space-y-12">
            ...
            <section>
                ...
                {/* Passing already fetched sports data to avoid redundant fetching inside component */}
            ...
            <section>
                ...
                <div className="flex overflow-x-auto snap-x hide-scrollbar -mx-4 px-4 gap-4 pb-4 sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 sm:gap-4 sm:pb-0 sm:mx-0 sm:px-0">
                    {/* Using Set.has (O(1)) instead of Array.find (O(M)) for efficient filtering */}
