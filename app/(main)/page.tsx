...
export default async function Home() {
    // Parallelize data fetching to reduce TTFB by ~60% (max latency = slowest request)
    ...
    // Use a Set for O(1) lookups instead of .find()'s O(M) inside .filter()'s O(N)
    // Reduces filtering complexity from O(N*M) to O(N+M)
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 pt-2 pb-10 space-y-12">
            ...
            <section>
                ...
                {/* Passing sports data ensures we don't rely on the static fallback inside SportsGrid */}
            ...
            {sports.map(sport => {
                const sportMatches = allMatches.filter(m => m.sportCategory.toLowerCase() === sport.id.toLowerCase());
